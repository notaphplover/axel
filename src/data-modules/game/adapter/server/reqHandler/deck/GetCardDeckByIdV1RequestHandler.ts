...
export class GetCardDeckByIdV1RequestHandler implements FastifyRequestHandler {
  ...
  constructor(
    @inject(GAME_ADAPTER_TYPES.api.converter.deck.CARD_DECK_TO_CARD_DECK_API_V1_CONVERTER)
    private readonly cardDeckToCardDeckApiV1Converter: Converter<CardDeck, CardDeckApiV1>,
  ...
  public async handle(
    ...
    if (findResult === null) {
      await reply
        ...
        .send(`The card deck with id "${findCardDeckQuery.id as string}" was not found`);
    } else {
      await reply.send(this.cardDeckToCardDeckApiV1Converter.transform(findResult));
    }
  }
}
