...
const getGameSetupsV1RequestHandler: FastifyRequestHandler = {
  handle: jest.fn(),
};
    ...
    describe('GET GameSetups', () => {
      describe('when called, with valid auth', () => {
        ...
        beforeAll(async () => {
          ...
          (getGameSetupsV1RequestHandler.handle as jest.Mock).mockImplementationOnce(
            async (
        ...
        afterAll(async () => {
          ...
          (getGameSetupsV1RequestHandler.handle as jest.Mock).mockClear();
        });
        ...
        it('must call getGameSetupsV1RequestHandler.handle to handle the request', () => {
          expect(getGameSetupsV1RequestHandler.handle).toHaveBeenCalledTimes(1);
      ...
      describe('when called, with wrong auth', () => {
        ...
        it('must not call getGameSetupsV1RequestHandler.handle to handle the request', () => {
          expect(getGameSetupsV1RequestHandler.handle).not.toHaveBeenCalled();
        });
